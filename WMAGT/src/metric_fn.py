...
def get_metrics_fpr(real_score, predict_score):
    import gc
    gc.collect()
...
def evaluate(predict, label, is_final=False):
    ...
    fpr, tpr, thresholds = metrics.roc_curve(y_true=label, y_score=predict)
    import csv;
    with open('FprAndTpr_WMAGT_C.csv', 'w', newline='') as newfile:
        writer = csv.writer(newfile)
        for i in range(len(fpr)):
            writer.writerow([fpr[i],tpr[i]])
    precision, recall, thresholds = metrics.precision_recall_curve(y_true=label, probas_pred=predict)
    with open('PR.csv', 'w', newline='') as newfile:
        writer = csv.writer(newfile)
        for i in range(len(fpr)):
