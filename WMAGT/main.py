...
def train_fn(config, model, train_loader, val_loader):
    ...
    trainer = Trainer(max_epochs=config.epochs,
                      ...
                      default_root_dir="demo-run",
                      profiler=config.profiler,
                      fast_dev_run=False,
                      checkpoint_callback=checkpoint_callback,
                      callbacks=[lr_callback],# 将自定义回调添加到回调列表中
                      gpus=config.gpus,
                      check_val_every_n_epoch=1,
                      # logger = logger  # 使用 TensorBoardLogger
                      )
    ...
    # files = [os.path.join(config.dirpath, file) for file in os.listdir(config.dirpath)]
    # files = [os.path.join("run", file) for file in os.listdir("run")]

    files = [os.path.join("demo-run/lightning_logs", file) for file in os.listdir("demo-run/lightning_logs")]
    path = files[-1]+"/checkpoints"
...
def train(config, model_cls=WMAGT):
    ...
    config.drug_edge_len = datasets.drug_edge[1].size()
...
def parse(print_help=False):
    ...
    parser = WMAGT.add_model_specific_args(parser)
